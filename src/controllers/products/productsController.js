...
const listProducts = async (req, res) => {
  ...
  try {
    ...
    find = channel_id ? { ...find, id_owner: channel_id } : { ...find };
    ...
    if (filtros_type.length > 0) {
      ...
      for (let i = 0; i < filtros_type.length; i++) {
        ...
        if (
          regra_filtros.includes(filtros_type[i]._id) &&
          filtros_type[i]._id.length > 0
        ) {
...
const findProductById = async (req, res) => {
  ...
  try {
    ...
    console.log("product: ", product.id_owner);
    console.log("userId: ", userId);
    if (String(product.id_owner) === String(userId)) {
      return res.status(200).json({
        data: product,
      });
    } else {
      return res.status(500).send({
        message: "ERRO ao listar products: produto não encontrado",
        error: {},
      });
    }
  } catch (error) {
...
const registerProduct = async (req, res) => {
  ...
  let files = req.files;
  let image_product = files.filter((file) => {
    return file.fieldname == "imageurl";
  });
  ...
  if (data.stickersinfo.length > 0 && files) {
    ...
    for (let i = 0; i < quant_stickers; i++) {
      ...
      let image_sticker = files.filter((file) => {
        return file.fieldname == `stickers_${slot_sticker}`;
      });
...
const editProduct = async (req, res) => {
  try {
    ...
    const id = req.params.id;
    let product = await Products.findById(id);
    if (product && String(product.id_owner) === String(userId)) {
      let data = req.body;
      let files = req.files;
      ...
      if (data.quant_stickers) {
        quant_stickers = data.quant_stickers
          ? parseInt(data.quant_stickers)
          : 0;
        data.quant_stickers = quant_stickers;
      }
      ...
      let stickersinfo_temp = data.stickersinfo
        ? JSON.parse(data.stickersinfo)
        : [];
      if (stickersinfo_temp.length > 0) {
        ...
        if (data.stickersinfo.length > 0 && files) {
          ...
          for (let i = 0; i < quant_stickers; i++) {
            let slot_exist = stickersinfo_old.filter((stiker) => {
              return stiker.slot == data.stickersinfo[i].slot;
            });
            let sticker_index = stickersinfo_old.findIndex(
              (sticker) => sticker.slot == data.stickersinfo[i].slot
            );
            let image_sticker = files.filter((file) => {
              return file.fieldname == `stickers_${data.stickersinfo[i].slot}`;
            });
            ...
            //se apenas o nome do sticker for editada e existir um path_img
              ...
              slot_exist[0] &&
              slot_exist[0].path_img &&
              slot_exist[0].path_img.length > 0 &&
              data.stickersinfo[i].name.length > 0
            ) {
              console.log(
                "se apenas o nome do sticker for editada e existir um path_img"
              );
            ...
            //se apenas o nome do sticker for editada e existir um link_img
              ...
              slot_exist[0] &&
              slot_exist[0].link_img &&
              slot_exist[0].link_img.length > 0 &&
              data.stickersinfo[i].name.length > 0
            ) {
              console.log(
                "se apenas o nome do sticker for editada e existir um link_img"
...
const setPromo = async (req, res) => {
  ...
  try {
    console.log("req.userId: ", req.userId);
    const id_user = req.userId;
    let person = await Pessoa.findById(id_user).populate(
  ...
  try {
    ...
    if (product) {
      ...
      for (let i = 0; i < product.stickersinfo.length; i++) {
        if (
          product.stickersinfo[i].path_img &&
          product.stickersinfo[i].path_img.length > 0
        ) {
...
const deleteStickerProduct = async (req, res) => {
  ...
  try {
    ...
    let sticker = product.stickersinfo.filter(
      (sticker_) => sticker_.slot == slot
    );
    console.log("sticker: ", sticker);
    let index_sticker = product.stickersinfo.findIndex(
      (sticker_) => sticker_.slot == slot
    );
...
const historyRedeemProduct = async (
  id_user,
  id_product,
  id_owner,
  id_channel
    ...
    try {
      ...
      if (product && person) {
        let redeem = {
          ...
          product_float:
            parseFloat(product.floatvalue) > 0 ? product.floatvalue : null,
          id_user: person._id,
          tradeLink:
            person.tradelinkSteam.length > 0 ? product.tradelinkSteam : null,
          id_owner: id_owner,
  ...
  try {
    ...
    if (product) {
      ...
      if (index_channel != -1) {
        if (String(id_channel).length > 0) {
          if (person.tradelinkSteam && person.tradelinkSteam.length > 0) {
            if (product.status == "emEstoque") {
              let price_real = product.promo
                ? product.pricePromo
                : product.price;
              console.log("price_real: ", price_real);
              let points_in_channel = person.channels[index_channel].points;
              if (points_in_channel >= price_real) {
                                    ...
                                    <body>
                                        <div class="content-email">
                                            ...
                                            <div class="containerButton">
                                                <a class="buttonEntrega" href="${
                                                  process.env.URL_SITE
                                                }/dashboard/resgateProdutosPendentes""  >Entrega produto</a>
                                            </div>
                                            <div class="content-info">
                                                <p class="info-product"><b>Nome do produto:</b> ${
                                                  product.name
                                                    ? product.name
                                                    : "Não cadastrado"
                                                } </p><br>
                                                <p class="info-product"><b>Float do produto:</b> ${
                                                  product.floatvalue
                                                    ? product.floatvalue
                                                    : "Não cadastrado"
                                                } </p><br>
                                                <p class="info-product"><b>Desgaste do produto:</b> ${
                                                  product.exterior
                                                    ? product.exterior
                                                    : "Não cadastrado"
                                                } </p><br>
                                                <p class="info-product"><b>Trade link do usuário:</b> ${
                                                  person.tradelinkSteam
                                                    ? person.tradelinkSteam
                                                    : "Não cadastrado"
                                                } </p><br>
                                                <p class="info-product"><b>Valor do produto:</b> ${
                                                  product.price
                                                    ? product.price
                                                    : "Não cadastrado"
                                                } </p><br>
                  ...
                  attachments: [
                    {
                      ...
                      path: product.imageurl
                        ? product.imageurl
                        : product.imagepath
                        ? process.env.URL_SERVER + "/" + product.imagepath
                ...
                if (product.amount > 1) {
                  let product_up = await Products.findByIdAndUpdate(
                    id_product,
                    {
                  ...
                  person.points = person.points - price_real;
                  person.channels[index_channel].points =
                    points_in_channel - price_real;
                  ...
                  let resp = await historyRedeemProduct(
                    id_user,
                    id_product,
                    id_owner,
                    id_channel
                  ...
                  if (resp) {
                    ...
                    person.channels[index_channel].points =
                      points_in_channel + price_real;
                    await person.save();
                    await Products.findByIdAndUpdate(id_product, {
                      amount: product.amount + 1,
                    });
                    return res.status(500).send({
                      message:
                        "Erro ao resgatar produto, erro ao cadastro log de resgate",
                    });
                  }
                } else {
                  ...
                  if (product.amount > 0) {
                    ...
                    person.channels[index_channel].points =
                      points_in_channel - price_real;
                    await person.save();
                    await Pessoa.findByIdAndUpdate(id_user, {
                    ...
                    return res.status(400).send({
                      message:
                        "Erro ao resgatar produto, produto fora de estoque",
                    });
                  }
                }
              } else {
                return res.status(400).send({
                  message:
                    "Erro ao resgatar produto, pontos insuficientes neste canal",
                });
              }
            } else {
              return res.status(400).send({
        ...
        return res.status(400).send({
          message: "Erro ao resgatar produto, não possui pontos neste canal",
        });
      }
    } else {
      return res.status(400).send({
