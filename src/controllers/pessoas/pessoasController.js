...
const listPessoas = async (req, res) => {
  try {
    ...
    let pessoa = await Pessoa.findById(id_user).populate(
      "permissions.ifo_permission"
    );
    let perm_admin = pessoa
      ? pessoa.permissions.findIndex((permisao) => {
...
const editPerson = async (req, res) => {
  try {
    const id_user = req.userId;
    const id_user_edit =
      req.params && req.params.id_user ? req.params.id_user : "";
    let pessoa = await Pessoa.findById(id_user_edit).populate(
      "permissions.ifo_permission"
    );
...
const listPessoasOn = async (req, res) => {
  try {
    let pessoasOn = await Pessoa.find({ "channels.status": { $in: true } })
      .populate("channels.info_channel")
      .populate("permissions.ifo_permission");
    res.status(200).json({
      data: pessoasOn,
...
const registerPerson = async (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      let person_exists = await Pessoa.find({
...
const addChannel = async (req, res) => {
  ...
  try {
    let exist_channel_in_user = await Pessoa.find({
      "channels.info_channel": { $in: channel_id },
    }).where({ _id: id_user });
    ...
    if (exist_channel_in_user.length == 0) {
      if (exist_channel) {
        ...
        let pessoa = await Pessoa.findByIdAndUpdate(id_user, {
          channels: data,
        });
        res.status(201).json({
      ...
      if (person.type_account == "pendente") {
        ...
        if (type_account == "secondary" && id_person_primary) {
          ...
          if (person_primary.secondary_accounts.length >= 2) {
            return res.status(400).json({
              message:
                "Conta primária já excedeu o limite de contas secundárias vinculadas",
            });
          }
        ...
        if (type_account == "primary") {
          ...
          let persons_primary = await Pessoa.find({
            ip_user: ip,
          ...
          if (persons_primary.length > 0) {
            return res.status(400).json({
              message:
                "Erro ao setar o tipo de conta, limite de contas primarias esgotado",
            });
          } else {
            if (codigo != null) {
...
const zerarPontosPessoas = async (req, res) => {
  try {
    ...
    const delete_points_user_channel = await Pessoa.updateMany(
      {},
      { $set: { "channels.$[].points": 0 } }
    );
    res.status(201).json({
...
const setStatusPessoaCanal = async (req, res) => {
  ...
  try {
    ...
    let index_channel = pessoa_old.channels.findIndex((channel) => {
      console.log(
        `channel_id (${channel_id}) :: channel.info_channel (${channel.info_channel})`
      );
...
const setPointPessoaCanal = async (id_user, channel_id, points) => {
  ...
  try {
    ...
    if (index_channel != -1) {
      pessoa.channels[index_channel].points =
        pessoa.channels[index_channel].points + 3;
      return pessoa.save();
    } else {
...
const setAccountLink = async (req, res) => {
  ...
  try {
    ...
    if (pessoa && accountLink_) {
      pessoa.accountsLinks = [...pessoa.accountsLinks, accountLink];
      ...
      res.status(400).json({
        message:
          "Erro setar conta vinculada, usuário ou conta não encontrado(a)",
      });
    }
  } catch (error) {
    res.status(400).json({
...
const restoreMongo = async (req, res) => {
  try {
    ...
    switch (schema) {
      case "pessoa":
        file.map(async (elem) => {
          ...
          await Pessoa.create(elem, function (err, res) {
            //collection
            if (err) throw err;
          });
      ...
      case "channel":
        ...
        file2.map(async (elem) => {
          elem._id = ObjectID(elem._id);
          await Channel.create(elem, function (err, res) {
            //collection
            if (err) throw err;
          });
          // console.log("Elemento: "+elem._id)
      ...
      case "accountslink":
        file3.map(async (elem) => {
          elem._id = ObjectID(elem._id);
          await AccountsLink.create(elem, function (err, res) {
            //collection
            if (err) throw err;
          });
          // console.log("Elemento: "+elem._id)
      ...
      case "rewards":
        file4.map(async (elem) => {
          ...
          await Rewards.create(elem, function (err, res) {
            //collection
            if (err) throw err;
          });
          // console.log("Elemento: "+elem._id)
      ...
      case "redeempoints":
        file5.map(async (elem) => {
          ...
          await RedeemPoints.create(elem, function (err, res) {
            //collection
            if (err) throw err;
          });
          // console.log("Elemento: "+elem._id)
      ...
      case "permissions":
        file6.map(async (elem) => {
          ...
          await Permissions.create(elem, function (err, res) {
            //collection
            if (err) throw err;
          });
          // console.log("Elemento: "+elem._id)
        ...
        file7.forEach(async (elem) => {
          try {
            ...
            for (let i = 0; i < elem.stickersinfo.length; i++) {
              elem.stickersinfo[i]._id = ObjectID(
                elem.stickersinfo[i]._id.$oid
              );
            }
    ...
    res
      .status(500)
      .json({ message: "Erro ao fazer o restore para o schema " + schema });
  }
};

const checkExistUserAdmin = async () => {
  return new Promise(async (resolve, reject) => {
    try {
      let admin_exist = await Pessoa.find({ nickname: "admin_system" });
      let permissions = [];
      let permissions_exist = await Permissions.find();
      let data = {
        streamer: true,
      ...
      if (permissions_exist.length > 0) {
        if (admin_exist.length == 0) {
          for (let i = 0; i < permissions_exist.length; i++) {
            permissions = [
              ...permissions,
              {
                ifo_permission: permissions_exist[i].id,
              },
            ];
          }
          data.permissions = permissions;
          let cad_admin = await authController.createAuthStreamer(data);
          if (cad_admin.status) {
            console.log("usuario admin criado com sucesso");
            resolve(true);
          } else {
            console.log(
              "Erro ao criar usuario admin: message= ",
              cad_admin.message,
              " - erro = ",
              cad_admin.error
            );
          ...
          console.log("usuario admi e permissoes ja criados");
          resolve(true);
        }
      } else {
        let permission_amdin = await permissionControler.createPermission(
          "admin",
          1
        );
        let permission_streamer = await permissionControler.createPermission(
          "streamer",
          2
        );
        let permission_viewer = await permissionControler.createPermission(
          "viewer",
          3
        );
        data.permissions = [
          {
            ifo_permission: permission_amdin.id,
          },
          {
            ifo_permission: permission_streamer.id,
          },
          {
            ifo_permission: permission_viewer.id,
          },
        ];
        let cad_admin = await authController.createAuthStreamer(data);
        console.log("usuario admi e permissoes criados com sucesso");
        resolve(true);
      }
    } catch (error) {
      console.log("Erro ao criar usuario admin: ", error);
      resolve(true);
    }
  });
};
